...
gst_queuex_finalize (GObject * object)
{
  ...
  while (!g_queue_is_empty (&queue->queue)) {
    GstQueuexItem *qitem = g_queue_pop_head (&queue->queue);

    if (qitem->type != GST_QUEUEX_ITEM_TYPE_QUERY)
      gst_mini_object_unref (qitem->item);
    g_slice_free (GstQueuexItem, qitem);
  }
...
static GstMessage *
gst_queuex_get_buffering_message (GstQueuex * queue)
{
  GstMessage *msg = NULL;
...
gst_queuex_post_buffering (GstQueuex * queue)
{
  ...
  msg = gst_queuex_get_buffering_message (queue);
    ...
    GstQueuexItemType item_type)
{
  ...
  if (item) {
    ...
    if (QUEUE_IS_USING_QUEUE (queue)) {
      GstQueuexItem *qitem = g_slice_new (GstQueuexItem);
  ...
  switch (GST_QUERY_TYPE (query)) {
    default:
      if (GST_QUERY_IS_SERIALIZED (query)) {
        ...
        GST_QUEUEX_MUTEX_LOCK_CHECK (queue, queue->sinkresult, out_flushing);
        if (QUEUE_IS_USING_QUEUE (queue) && (gst_queuex_is_empty (queue)
                || !queue->use_buffering)) {
